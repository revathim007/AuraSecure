...
function HazardDetection() {
    ...
    if (value && isNaN(numValue)) {
      ...
      switch (name) {
        ...
        case 'smokeLevel':
          ...
          break;
        case 'temperature':
          if (numValue < -50 || numValue > 70.7) error = 'Must be between -50°C and 70.7°C.';
          break;
        default:
          break;
      }
  ...
  const handleDetectHazard = () => {
    ...
    axios.post('http://localhost:8000/api/user-input-sensor-data/', sensorData)
      .then(response => {
      ...
      .catch(error => {
        ...
        setMessage('Error: Could not send data.');
        setTimeout(() => setMessage(''), 3000);
      });
  };
      ...
      <div className="hazard-detection-card">
        ...
        <div className="input-container">
          <div className="input-group">
            ...
            <input type="text" value={gasLevel} onChange={handleInputChange(setGasLevel, 'gasLevel')} />
            {errors.gasLevel && <p className="error-text">{errors.gasLevel}</p>}
          </div>
          <div className="input-group">
            <label>Smoke Level</label>
            <input type="text" value={smokeLevel} onChange={handleInputChange(setSmokeLevel, 'smokeLevel')} />
            {errors.smokeLevel && <p className="error-text">{errors.smokeLevel}</p>}
          </div>
          <div className="input-group">
            <label>Temperature</label>
            <input type="text" value={temperature} onChange={handleInputChange(setTemperature, 'temperature')} />
